...
from muckrake.dataset import Dataset, load_config
...
def run_crawl(config_path: Path, output: Optional[str] = None):
    ...
    output_path = None if output in (None, "-") else Path(output)

    previous_artifact = get_latest_successful_artifact(dataset_name)
    previous_path = None
    if previous_artifact is not None:
        previous_path = get_artifact_store().resolve_path(previous_artifact.storage_key)
    elif output_path is not None:
        previous_path = output_path
    timestamps = load_timestamps(previous_path) if previous_path is not None else {}
